...
export default function FleetPage() {
  ...
  useEffect(() => {
    if (!token) return;
    fetchVehicles(token).then(setVehicles).catch(() => undefined);
    fetchDrivers(token).then(setDrivers).catch(() => undefined);
  }, [token]);
      ...
      <div className="space-y-12">
        ...
        <section className="space-y-8">
          <LuxuryTable
            ...
            columns={[
              { header: "Registry", accessor: "license_plate", className: "font-mono text-gold-500" },
              { header: "Vehicle No", accessor: "vehicle_number" },
              { header: "Classification", accessor: "vehicle_type" },
              { header: "Operation Status", accessor: "status" },
            ]}
          />

          <LuxuryTable
            title="Certified Operators"
            data={drivers.map((d) => ({ ...d, id: d.id, name: `${d.first_name} ${d.last_name}` }))}
            columns={[
              {
              ...
              { header: "License", accessor: "license_number", className: "text-[10px] text-white/40 font-mono" },
